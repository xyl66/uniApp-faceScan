			...
			<view class='cropper-content'>
				<view v-if="isShowImg" class="wx-corpper" :style="'width:'+cropperInitW+'px;height:'+cropperInitH+'px;background:#000'">
					<view class="wx-corpper-content" :style="'width:'+cropperW+'px;height:'+cropperH+'px;left:'+cropperL+'px;top:'+cropperT+'px'">
						...
						<view class="wx-corpper-crop-box" @touchstart.stop="contentStartMove" @touchmove.stop="contentMoveing"
				...
				ctx.draw(true, () => {
					...
					uni.canvasToTempFilePath({
						...
						success: function(res) {
							uni.hideLoading()
							// 成功获得地址的地方
							const url = conf.webConfig.apiUrl + '/api/userauth/bupuserico' // 
								...
								success: (uploadFileRes) => {
									...
									if (jsonRet.ReturnCode === 'SUCCESS') {
										...
										var JsonDesc = JSON.parse(jsonRet.JsonDesc),
											errMsg = jsonRet.ReturnCode === 'FACEIMGERROR' ? conf.getCodeToMsg(jsonRet.ReturnCode) + '(' +
											JsonDesc.error_msg +
